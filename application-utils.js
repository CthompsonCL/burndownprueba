...
utils.asanaRequest = function (path, callback) {
  const token = Cookies.getItem('scrum_token')
  ...
  $.ajax({
    ...
    success: callback,
    beforeSend: auth,
    error: function () { window.alert('Error! Maybe a wrong token?!\n\n') }
  });
};

utils.renderGraph = function () {
  const ctx = document.getElementById('chart')

  console.info('here', ctx)
